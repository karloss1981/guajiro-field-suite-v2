...
export function Lightbox({ photos, startIndex = 0, onClose }: LightboxProps) {
  ...
  return (
    <div
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.96)', zIndex: 999, display: 'flex', flexDirection: 'column', touchAction: 'none' }}
      onClick={onClose}
    >
      <div
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', background: 'rgba(4,9,28,0.9)', flexShrink: 0 }}
        onClick={e => e.stopPropagation()}
      >
        <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontSize: 14, fontWeight: 700, color: '#fff' }}>
          📷 {idx + 1} / {photos.length}
          {photo.photo_type && (
            <span style={{ background: (photo.photo_type === 'evidence' || photo.photo_type === 'before') ? '#ffbe0033' : '#00dc8533', borderRadius: 20, padding: '1px 8px', fontSize: 10, color: (photo.photo_type === 'evidence' || photo.photo_type === 'before') ? '#ffbe00' : '#00dc85', fontWeight: 700, marginLeft: 8 }}>
              {(photo.photo_type === 'evidence' || photo.photo_type === 'before') ? 'ANTES' : 'DESPUÉS'}
            </span>
          )}
